  ...
  return (lineItems || [])
    .map((li) => {
      const nameRaw = String(
        li?.description || li?.price?.product?.name || li?.price?.nickname || "Item"
      );
...
async function enrichOrderFromStripeIfMissing(orderRow, stripe) {
  const hasItems = Array.isArray(orderRow?.items)
...
export async function list(req, res) {
  ...
  try {
    ...
    for (const row of rows) {
      const needsItems = !row?.items || (Array.isArray(row.items) && row.items.length === 0);
